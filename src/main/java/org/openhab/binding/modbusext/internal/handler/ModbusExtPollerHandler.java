...
public class ModbusExtPollerHandler extends BaseBridgeHandler
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        ...
        if (THING_TYPE_HOLDING_POLLER.equals(thing.getThingTypeUID())) {
            if (runtime.isPulseMode()) {
                handlePulseCommand(runtime, writeCommand, localComms, endpoint);
                return;
            }
            writeHolding(runtime, writeCommand, localComms, endpoint);
            return;
        }
        logger.debug("Ignoring command {} for {}: this poller is read-only", command, channelUID);
    }

    private void handlePulseCommand(ModbusChannelRuntime runtime, Command command, ModbusCommunicationInterface localComms, ModbusExtEndpointHandler<?> endpoint) {
        ...
        if (!runtime.tryBeginPulse(feedback.get(), requested.get())) {
            logger.debug("Pulse not required or already active for channel {} (feedback={}, requested={})", runtime.uid(), feedback.get(), requested.get());
            return;
        }

        logger.debug("Starting {} ms pulse for channel {} (feedback={}, requested={})", runtime.pulseDurationMillis(), runtime.uid(), feedback.get(), requested.get());
        if (!submitHoldingBit(runtime, true, localComms, endpoint, () ->
                scheduler.schedule(() -> submitHoldingBit(runtime, false, localComms, endpoint, () -> {
                    runtime.finishPulse();
                    logger.debug("Pulse completed for channel {}", runtime.uid());
                }, failure -> {
                    runtime.finishPulse();
                    logger.error("CRITICAL: failed to reset pulse output OFF for channel {}: {}", runtime.uid(), failure);
                }), runtime.pulseDurationMillis(), java.util.concurrent.TimeUnit.MILLISECONDS), failure -> {
                    runtime.finishPulse();
    ...
    private void writeHolding(ModbusChannelRuntime runtime, Command command, ModbusCommunicationInterface localComms, ModbusExtEndpointHandler<?> endpoint) {
        ...
        Integer writeStart = runtime.writeStart();
        if (valueType == null || writeStart == null) return;
        boolean rmw = valueType == ValueType.BIT || valueType == ValueType.INT8 || valueType == ValueType.UINT8;
        ModbusRegisterArray data;
        if (rmw) {
            synchronized (lastPolledRegisterCache) {
                ModbusRegisterArray current = lastPolledRegisterCache.get();
                if (current == null) { logger.warn("Cannot write {} for channel {} because holding-register cache is not populated", valueType, runtime.uid()); return; }
                ...
                if (valueType == ValueType.BIT) {
                    ...
                    byte[] bytes = commandData.getBytes();
                    current = HoldingRegisterRmw.writeByte(current, relative, runtime.writeSubIndex(), bytes[bytes.length - 1]);
                }
                lastPolledRegisterCache.set(current);
                data = HoldingRegisterRmw.singleRegister(current, relative);
            }
        } else data = ModbusBitUtilities.commandToRegisters(command, valueType);
    ...
    private boolean submitHoldingBit(ModbusChannelRuntime runtime, boolean value, ModbusCommunicationInterface localComms, ModbusExtEndpointHandler<?> endpoint, Runnable success, java.util.function.Consumer<Object> failure) {
        Integer writeStart = runtime.writeStart();
        if (writeStart == null) return false;
        ModbusRegisterArray data;
        synchronized (lastPolledRegisterCache) {
            ModbusRegisterArray current = lastPolledRegisterCache.get();
            if (current == null) { logger.warn("Cannot write pulse bit for channel {} because holding-register cache is not populated", runtime.uid()); return false; }
            int relative = writeStart - config.start;
            current = HoldingRegisterRmw.writeBit(current, relative, runtime.writeSubIndex(), value);
            lastPolledRegisterCache.set(current);
            data = HoldingRegisterRmw.singleRegister(current, relative);
        }
        ModbusWriteRegisterRequestBlueprint request = new ModbusWriteRegisterRequestBlueprint(endpoint.getSlaveId(), writeStart, data, false, config.maxTries);
        localComms.submitOneTimeWrite(request, result -> success.run(), f -> failure.accept(f));
        return true;
    }
    ...
    private void submitHolding(ModbusChannelRuntime runtime, ModbusRegisterArray data, boolean rmw, ModbusCommunicationInterface localComms, ModbusExtEndpointHandler<?> endpoint) {
        Integer writeStart = runtime.writeStart(); if (writeStart == null) return;
        ModbusWriteRegisterRequestBlueprint request = new ModbusWriteRegisterRequestBlueprint(endpoint.getSlaveId(), writeStart, data, data.size() > 1, config.maxTries);
        localComms.submitOneTimeWrite(request, result -> { lastResult = null; lastResultTimestamp = 0; if (!rmw) lastPolledRegisterCache.set(null); logger.debug("Holding-register write succeeded for channel {}: {}", runtime.uid(), result); }, failure -> { lastPolledRegisterCache.set(null); logger.warn("Holding-register write failed for channel {}: {}", runtime.uid(), failure); });
    }

    @Override
    public synchronized void initialize() {
        unregisterPollTask(); config = getConfigAs(ModbusPollerConfig.class);
        ...
        if (registerPoll && config.length > ModbusConstants.MAX_REGISTERS_READ_COUNT) { updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.CONFIGURATION_ERROR, "Register poll length exceeds Modbus protocol limit"); return; }
        if (!registerPoll && config.length > ModbusConstants.MAX_BITS_READ_COUNT) { updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.CONFIGURATION_ERROR, "Bit poll length exceeds Modbus protocol limit"); return; }
        if (!buildChannelRuntimes(registerPoll)) return;
        ModbusExtEndpointHandler<?> endpoint = getEndpointHandler();
        if (endpoint == null) { updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.BRIDGE_OFFLINE, "Modbus endpoint is offline"); return; }
        ModbusCommunicationInterface localComms = endpoint.getCommunicationInterface();
        if (localComms == null) { updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.BRIDGE_OFFLINE, "Modbus endpoint communication interface is not initialized"); return; }
        comms = localComms;
        ModbusReadRequestBlueprint request = new ModbusReadRequestBlueprint(endpoint.getSlaveId(), functionCode, config.start, config.length, config.maxTries);
        if (config.refresh <= 0) { updateStatus(ThingStatus.ONLINE, ThingStatusDetail.NONE, "Polling disabled"); return; }
        pollTask = localComms.registerRegularPoll(request, config.refresh, 0, this, this); updateStatus(ThingStatus.ONLINE);
    }
    ...
    public void handle(AsyncModbusReadResult result) {
        ...
        for (ModbusChannelRuntime runtime : channelRuntimes) if (runtime.hasRead()) { var state = runtime.extract(result); if (runtime.shouldUpdate(state, now)) updateState(runtime.uid(), state); }
        ThingStatusInfo status = thing.getStatusInfo();
        if (status.getStatus() == ThingStatus.OFFLINE && status.getStatusDetail() == ThingStatusDetail.COMMUNICATION_ERROR) updateStatus(ThingStatus.ONLINE);
    }
    ...
    private boolean buildChannelRuntimes(boolean registerPoll) {
        ...
        var view = new ModbusChannelRuntime.ModbusPollerConfigView(config.start, config.length, registerPoll);
        for (Channel channel : thing.getChannels()) try { runtimes.add(ModbusChannelRuntime.create(channel, view)); } catch (IllegalArgumentException e) { updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.CONFIGURATION_ERROR, e.getMessage()); return false; }
        channelRuntimes = List.copyOf(runtimes); return true;
    }
